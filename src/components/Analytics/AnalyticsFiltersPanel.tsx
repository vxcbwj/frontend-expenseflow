...
const AnalyticsFiltersPanel: React.FC<AnalyticsFiltersPanelProps> = ({
  ...
  const canViewAnalytics = React.useMemo(() => {
    // Always allow if user exists (basic check)
    if (!user) return false;

    // Get user's global role
    const userRole = user.globalRole?.toLowerCase() || user.role?.toLowerCase();

    // Allow super admins, company owners, and company admins
    if (
    ...
    // For other roles, use the permission system
    return hasPermission(user, "canViewAnalytics", currentCompany?.id || "");
  }, [user, currentCompany]);
