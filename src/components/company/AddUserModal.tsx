// components/company/AddUserModal.tsx - FIXED IMPORT
...
const AddUserModal: React.FC<AddUserModalProps> = ({
  ...
  const getAvailableRoles = () => {
    const userRoleInCompany = user?.companyRoles?.find(
    ...
    const baseRoles = [
      {
        value: "member",
        label: "Team Member",
        description: "Can create and manage their own expenses",
      },
      {
        value: "manager",
        label: "Manager",
        description: "Can manage team expenses and budgets",
      },
      {
        value: "company_admin",
        label: "Company Admin",
        description: "Full access to company management (except ownership)",
      },
    ];

    // Only company owners can assign owner role
    if (userRoleInCompany === "owner") {
      return [
        {
          value: "owner",
          label: "Company Owner",
          description: "Full ownership and control of the company",
        },
        ...baseRoles,
      ];
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setTimeout(() => {
          setFormData({ email: "", role: "member", department: "" });
          setSuccessMessage("");
          onClose();
        }, 2000);
      } else {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                User must already have an ExpenseFlow account
              </p>
            </div>
                  ...
                  <div
                    ...
                    className={`flex items-start p-3 border rounded-lg cursor-pointer transition-all duration-200 ${
                      formData.role === role.value
                        ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-600 ring-1 ring-blue-500/20"
            ...
            <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                ...
                <span className="font-medium">
                  {user?.companyRoles?.find((cr) => cr.companyId === companyId)
                    ?.role || "Not a member"}
                </span>
              </p>
            </div>
        ...
        <div className="px-6 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            ...
            added to the company's team immediately.
          </p>
        </div>
      </div>
    </div>
